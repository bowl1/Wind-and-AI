...
def run_federated_kmeans(
  ...
  # Use PCA for visualization
    ...
    if save_outputs:
        ...
        # getting the counts of each cluster
        cluster_counts = Counter(labels)

        # Create a new Series for the labels with counts
        df["cluster_label"] = df["cluster"].apply(
            lambda x: f"Cluster {x} ({cluster_counts[x]})"
        )

        # Redraw the plot (with the counted labels)
        ax = sns.scatterplot(
            x=X_tsne[:, 0],
            y=X_tsne[:, 1],
...
def grid_search_federated_kmeans():
    feature_csv = (
        # # first try the full set of 50 turbines
        # "./selected feature statistics/features_in_group_stage_50_turbines.csv"
        # second try the filtered set of 50 turbines (remove those outliner groups and small groups)
        # "./group_stage_results_new/2nd/features_cluster_0_1st.csv"
        "./group_stage_results_new/3rd/features_cluster_0_2nd.csv"
    )
    coord_csv = "./selected turbines/selected_50_turbines_filtered.csv"

    best_score = -1
    best_params = None
    results = []

    for n_clients in range(3, 8):
        for k_global in range(3, 6):
            for crounds in range(3, 8):
                print(
