...
def grid_search_federated_kmeans():
    feature_csv = (
        # first try
        # second try the full set of 200 turbines
        # "./group_stage_results/second_group/features_cluster_0_1st.csv"
        # third try the filtered set of 200 turbines
        "./group_stage_results/third_group/features_cluster_0_2nd.csv"
    )
    coord_csv = "./selected turbines/selected_200_turbines_filtered.csv"

    best_score = -1
    best_params = None
    results = []

    for n_clients in range(5, 10):
        for k_global in range(3, 10):
            for crounds in range(3, 10):
                ...
                try:
                    ...
                    if score >= best_score:
                        ...
                        best_params.append((n_clients, k_global, crounds))
