...
# ===== DRS initialization function (Double Roulette Sampling) in server side ===== 
...
# ===== Client =====
class client_FKM: # kmeans++ initialization and k-means grouping in client side
    def __init__(self, data, n_clusters, n_iter=1): # n_iter: local iterations
        self.data = data
        self.n_clusters = min(n_clusters, len(data))  # ensure n_clusters <= number of samples
        self.n_iter = n_iter
        self.means = None # current centers

    def compute_d2(self, current_centers):  # Compute the squared distance to the nearest center
        dists = np.min(np.linalg.norm(self.data[:, None] - current_centers[None, :], axis=2)**2, axis=1)
        return dists

    def sample_point_by_d2(self, current_centers): # Sample a point based on D^2 probability, which is k-means++ initialization
        d2 = self.compute_d2(current_centers)
        probs = d2 / np.sum(d2)
        idx = np.random.choice(len(self.data), p=probs)
        return self.data[idx]

    def det_local_clusters(self): # Determine local clusters using current means(center), k-means in each client only run 1 iteration
...
class server_FKM:
    ...
    def aggregate(self, local_clusters, cluster_sizes):
        kmeans = KMeans(n_clusters=self.n_global)  # use k-means to aggregate local centers, because local centers sample distributions are imbalanced, we use k-means to balance
        kmeans.fit(local_clusters, sample_weight=cluster_sizes)
        return kmeans.cluster_centers_ # updated global centers based on weighted local centers
...
# ==== Extract GSRN and Feature Columns for Clustering ====
feature_cols = [
    "mean_power_scaled",
    "std_power_scaled",
    "cv_scaled",
...
# # ==== group clients (every 5 GSRN combine into one client based on geo location) ====

# 1. aquiring unique GSRNs
...
n_clients = 3 # the number of clients
k_global = 4 # the global cluster number
...
print("\n=== Cluster Feature Standard Deviations ===")
print(cluster_stds)
