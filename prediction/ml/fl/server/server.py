...
class Server:
    ...
    def _initialize_client_manager(self, client_manager) -> None:  #客户端管理器初始化
    ...
    def fit_round(self, fl_round: int,   #每一轮训练逻辑
                  fraction: float,
    ...
    def sample_clients(self, fl_round: int, fraction: float,   #客户端抽样
    ...
    def aggregate_models(self, fl_round: int, results: List[Tuple[List[np.ndarray], int]]) -> List[np.ndarray]:  #模型聚合  
        log(INFO, f"[Global round {fl_round}] Aggregating local models...")
        aggregated_params = self.aggregator.aggregate(results, self.global_model) #取决于开始传入的模型参数
