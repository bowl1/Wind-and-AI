...
def read_data(data_path: str, filter_data: str = None) -> pd.DataFrame:
    ...
    if filter_data is not None:
        ...
        df = df.loc[df['GSRN'] == filter_data]
        
    df.rename(columns={"timestamp": "time"}, inplace=True)
    df.set_index(pd.DatetimeIndex(df["time"]), inplace=True)

    # Exclude non-float columns
    non_float_cols = ["GSRN", "cluster"]
    float_cols = [col for col in df.columns if col not in non_float_cols]
...
def handle_nans(train_data: pd.DataFrame,
                ...
                identifier: Optional[str] = "GSRN") -> Union[pd.DataFrame, Tuple[pd.DataFrame, pd.DataFrame]]:
...
def time_to_feature(df: pd.DataFrame,
                    ...
                    identifier: str = "GSRN") -> Union[pd.DataFrame, None]:
...
def scale_features(train_data: pd.DataFrame,
                   ...
                   val_data: Optional[pd.DataFrame] = None,
                   per_area: bool = False,
                   identifier: str = "GSRN") -> Union[pd.DataFrame, pd.DataFrame, Any]:
...
def to_train_val(df: pd.DataFrame,
                 ...
                 identifier: str = "GSRN") -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def get_exogenous_data_by_area(exogenous_data_train: pd.DataFrame,
                               ...
                               identifier: Optional[str] = "GSRN") -> Union[
...
class TimeSeriesDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.exogenous is None:
            empty_exo = torch.zeros((self.X[index].shape[0], 0), dtype=self.X.dtype)
            return self.X[index], empty_exo, y_hist, self.y[index]
        else:
