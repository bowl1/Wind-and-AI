...
class LSTM(torch.nn.Module):
    ...
    def forward(self, x, exogenous_data=None, device="cpu", y_hist=None):
         # 只在最后一维是 1 的时候再 squeeze，否则保持原样
        if x.dim() == 4 and x.shape[-1] == 1:
            x = x.squeeze(-1)  # (B, lags, features)
            
         # 不要 reshape 或 view，这样会破坏时间维度，如果你启用 matrix_rep 模式才 reshape
        if self.is_matrix:
...
# Append exogenous data if provided, if exogenous_data is none then skip
        if exogenous_data is not None and self.is_matrix and exogenous_data.numel() > 0:
            if exogenous_data.dim() > 2:
                exogenous_data = exogenous_data.view(exogenous_data.size(0), -1)
